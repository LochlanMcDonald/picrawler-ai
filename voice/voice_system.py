...
class VoiceSystem:
    ...
    def __init__(self, config: dict, cache_dir: str = "logs/tts_cache"):
        ...
        # OpenAI client: prefer config key, fall back to env var, else a
        # placeholder so construction never crashes (say() fails non-fatally).
    ...
    def _is_in_cache_dir(self, path: Path) -> bool:
        """True if path resolves to a location inside the cache directory."""
        try:
            resolved = path.resolve()
            cache = self.cache_dir.resolve()
            return os.path.commonpath([resolved, cache]) == str(cache)
    ...
    def _get_or_create_audio(self, text: str) -> Path:
        ...
        # Security: cache file must stay inside the cache directory
        if not self._is_in_cache_dir(out):
        ...
        audio = self.client.audio.speech.create(
            ...
            input=text,
            response_format=self.settings.format,
    ...
    def _play(self, audio_path: Path) -> None:
        # Security: only ever play files from our own cache directory
        if not self._is_in_cache_dir(audio_path):
        ...
        if not audio_path.resolve().exists():
